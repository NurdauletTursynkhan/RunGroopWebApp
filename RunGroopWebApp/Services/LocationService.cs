...
namespace RunGroopWebApp.Services
{
    public class LocationService : ILocationService
    {
        ...
        public async Task<List<City>> GetLocationSearch(string location)
        {
            ...
            {
                return await _context.Cities.Where(x => x.Zip.ToString().StartsWith(location))
                                            .Take(4)
                                            .Distinct()
                                            .ToListAsync();
            }
            ...
            {
                return await _context.Cities.Where(x => x.CityName == location)
                                            .Take(10)
                                            .ToListAsync();
            }

            return await _context.Cities.Where(x => x.StateCode == location)
                                        .Take(10)
                                        .ToListAsync();
        }
    }
